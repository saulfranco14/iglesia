 ...
 *  @final
 */
final Class Talla extends Base implements peticiones
{
	/*
	...
	/*
	 * Atributo privado para la id del talla
	 * @access private
	 * @var int
	 */
	private  $_id_talla;
	...
	/*
	 * Atributo privado para la creacion del talla
	 * @access private
	 * @var string
	 */
	private $_created_at;
	...
	/*
	 * Atributo privado para la actualizacion del talla
	 * @access private
	 * @var string
	 */
	private $_updated_at;
	...
	/*
	 * Atributo privado para el nombre del talla
	 * @access private
	 * @var string
	 */
	private $_nombre;
	...
	/*
	 * Atributo privado para el orden del talla
	 * @access private
	 * @var string
	 */
	private $_orden;
	...
	/*
	 * Atributo privado para el descripcion del talla
	 * @access private
	 * @var string
	 */
	...
	public function __construct()
	{
		...
		if(parent::validaSesion()){
			...
			$this->_columnas = array('', 'talla.id_talla','talla.created_at','talla.updated_at','talla.nombre','talla.orden','talla.descripcion');
			$this->_aliasColumnas =' talla.id_talla AS a, talla.created_at AS b,talla.updated_at AS c, talla.nombre AS d, talla.orden AS e, talla.descripcion AS f ';
		}else {
	...
	/*
	 * funcion para listar los registros de la tabla talla.
	 * @access public
	...
	public function listar($tipo)
	{
		...
		$this->_sql = sprintf("SELECT %s FROM talla WHERE talla.id_talla>=1 ORDER BY talla.id_talla DESC;",$this->_aliasColumnas);
	...
	public function buscar($criterio,$columna,$tipo)
	{
		...
		if ($columna == 0){
			...
			$this->_id_talla 			= $this->formatear($criterio, "CadenaBusqueda");
			$this->_nombre 				= $this->formatear($criterio, "CadenaBusqueda");

			$this->_sql = sprintf("SELECT %s FROM talla WHERE (talla.id_talla LIKE %s OR talla.nombre LIKE %s) ORDER BY talla.orden DESC;",$this->_aliasColumnas, $this->_id_talla, $this->_nombre);

		}else {

			$this->_columna 	= $this->_columnas[$columna];
			$this->_criterio 	= $this->formatear($criterio, "CadenaBusqueda");
			$this->_sql 		= sprintf("SELECT %s FROM talla WHERE %s LIKE %s ORDER BY talla.orden DESC;",
	...
	/*
	 * Funcion para insertar un registro en la tabla talla
	 * @access public
	...
	public function insertar($registro){
		...
		$this->_sql = sprintf("INSERT INTO talla VALUES(	NULL ,
	...
	public function actualizar($registro){
		...
		$this->_id_talla 		= $this->formatear($registro->a, "Entero");
		$this->_nombre 			= $this->formatear(strtoupper($registro->d), "Cadena");
		$this->_orden 			= $this->formatear($registro->e, "Cadena");
		$this->_descripcion 	= $this->formatear(strtoupper($registro->f), "Cadena");
		
		$this->_sql 			= sprintf("UPDATE talla SET  talla.updated_at=NOW(), talla.nombre=%s, talla.orden=%s, talla.descripcion=%s WHERE talla.id_talla=%s LIMIT 1;",$this->_nombre,$this->_orden,$this->_descripcion,$this->_id_talla);
		return $this->sentenciaSQL($this->_sql, 5);
	}
	
	/*
	 * Funcion Eliminar un registro en la tabla talla
	 * @access public
	...
	public function borrar($registro) 
	{
		...
		$this->_sql 		= sprintf("DELETE FROM talla WHERE talla.id_talla=%s LIMIT 1;",$this->_id_talla);
		return $this->sentenciaSQL($this->_sql, 5);
	 ...
	 public function reportePDF($tipo, $criterio='', $columna='')
        {
          ...
          $_nombre = 'lista_talla.pdf';
          $_titulo = 'Listado de tallas';
          break;
          case 'buscar':
          //busqueda
          $this->_resultSet = $this->buscar($criterio, $columna,0);
          $_nombre = 'busqueda_talla.pdf';
          $_titulo = 'Busqueda de talla por "'.$criterio . '"';
          break;
          default:

 ...
 *  @final
 */
final Class Modelo extends Base implements peticiones
{
	/*
	...
	/*
	 * Atributo privado para la id del modelo
	 * @access private
	 * @var int
	 */
	private  $_id_modelo;
	...
	/*
	 * Atributo privado para la creacion del modelo
	 * @access private
	 * @var string
	 */
	private $_created_at;
	...
	/*
	 * Atributo privado para la actualizacion del modelo
	 * @access private
	 * @var string
	 */
	private $_updated_at;
	...
	/*
	 * Atributo privado para el nombre del modelo
	 * @access private
	 * @var string
	 */
	private $_nombre;
	...
	/*
	 * Atributo privado para el numero del modelo
	 * @access private
	 * @var string
	 */
	private $_numero;
	...
	/*
	 * Atributo privado para el descripcion del modelo
	 * @access private
	 * @var string
	 */
	...
	public function __construct()
	{
		...
		if(parent::validaSesion()){
			...
			$this->_aliasColumnas =		'modelo.id_modelo 		AS a,
	...
	/*
	 * funcion para listar los registros de la tabla modelo.
	 * @access public
	...
	public function listar($tipo)
	{
		...
		$this->_sql = sprintf("SELECT %s FROM modelo WHERE modelo.id_modelo>=1 ORDER BY modelo.id_modelo DESC;",$this->_aliasColumnas);
	...
	public function buscar($criterio,$columna,$tipo)
	{
		...
		if ($columna == 0){
			...
			$this->_sql = sprintf("SELECT %s FROM modelo WHERE (modelo.id_modelo LIKE %s OR modelo.nombre LIKE %s) ORDER BY modelo.numero DESC;",$this->_aliasColumnas, $this->_id_modelo, $this->_nombre);

		}else {

			$this->_columna 	= $this->_columnas[$columna];
			$this->_criterio 	= $this->formatear($criterio, "CadenaBusqueda");
			$this->_sql 		= sprintf("SELECT %s FROM modelo WHERE %s LIKE %s ORDER BY modelo.numero DESC;",
	...
	/*
	 * Funcion para insertar un registro en la tabla modelo
	 * @access public
	...
	public function insertar($registro){
		...
		$this->_numero 			= $this->formatear($registro->e, "Cadena");
		$this->_descripcion 	= $this->formatear(strtoupper($registro->f), "Cadena");

		$this->_sql = sprintf("INSERT INTO modelo VALUES(	NULL ,
	...
	public function actualizar($registro){
		...
		$this->_id_modelo 		= $this->formatear($registro->a, "Entero");
		$this->_nombre 			= $this->formatear(strtoupper($registro->d), "Cadena");
		$this->_numero 			= $this->formatear($registro->e, "Cadena");
		$this->_descripcion 	= $this->formatear(strtoupper($registro->f), "Cadena");
		
		$this->_sql 			= sprintf("UPDATE modelo SET  modelo.updated_at=NOW(), modelo.nombre=%s, modelo.numero=%s, modelo.descripcion=%s WHERE modelo.id_modelo=%s LIMIT 1;",$this->_nombre,$this->_numero,$this->_descripcion,$this->_id_modelo);
		return $this->sentenciaSQL($this->_sql, 5);
	}
	
	/*
	 * Funcion Eliminar un registro en la tabla modelo
	 * @access public
	...
	public function borrar($registro) 
	{
		...
		$this->_sql 		= sprintf("DELETE FROM modelo WHERE modelo.id_modelo=%s LIMIT 1;",$this->_id_modelo);
		return $this->sentenciaSQL($this->_sql, 5);
	 ...
	 public function reportePDF($tipo, $criterio='', $columna='')
        {
          ...
          $_nombre = 'lista_modelo.pdf';
          $_titulo = 'Listado de Modelos';
          break;
          case 'buscar':
          //busqueda
          $this->_resultSet = $this->buscar($criterio, $columna,0);
          $_nombre = 'busqueda_modelo.pdf';
          $_titulo = 'Busqueda de Modelo por "'.$criterio . '"';
          break;
          default:

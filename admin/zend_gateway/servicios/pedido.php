 ...
 *  @final
 */
final Class Pedido extends Base implements peticiones
{
	/*
	...
	*/
	public function __construct()
	{
		//verificar si esta logeado
			...
			$this->_aliasColumnas ='
				...
				pedido.id_cliente_pagina 	AS a,
			 	pedido.created_at 			AS b,
			 	pedido.updated_at 			AS c,
			 	pedido.nombre 				AS d,
			 	pedido.telefono 			AS e,
			 	pedido.email 				AS f,
			 	pedido.comentario 			AS g,
			 	pedido.origen 				AS h ';
	...
	public function listar($tipo)
	{
		...
		$this->_sql  = sprintf("SELECT %s FROM pedido WHERE pedido.id_cliente_pagina>=1 ORDER BY pedido.id_cliente_pagina DESC;",$this->_aliasColumnas);
	...
	public function buscar($criterio,$columna,$tipo)
	{
		...
		if ($columna == 0){
			...
			$this->_id_cliente_pagina 			= $this->formatear($criterio, "CadenaBusqueda");
			$this->_nombre 						= $this->formatear($criterio, "CadenaBusqueda");
			$this->_telefono 					= $this->formatear($criterio, "CadenaBusqueda");
			$this->_email 						= $this->formatear($criterio, "CadenaBusqueda");
			$this->_comentario 					= $this->formatear($criterio, "CadenaBusqueda");
			$this->_origen 						= $this->formatear($criterio, "CadenaBusqueda");



			$this->_sql = sprintf("SELECT %s FROM pedido WHERE (

									pedido.id_cliente_pagina  LIKE %s OR
									pedido.nombre 	   		  LIKE %s OR
									pedido.telefono 	   	  LIKE %s OR
									pedido.email 	   		  LIKE %s OR
									pedido.comentario 	   	  LIKE %s OR
									pedido.origen 	   		  LIKE %s 
									)
									ORDER BY pedido.id_cliente_pagina DESC;",

									$this->_aliasColumnas,
									$this->_id_cliente_pagina,
									$this->_nombre,
									$this->_telefono,
									$this->_email,
									$this->_comentario,
			...
			$this->_sql 		= sprintf("SELECT %s FROM pedido WHERE %s LIKE %s ORDER BY pedido.id_cliente_pagina DESC;",
	...
	public function insertar($registro){
		...
		$this->_telefono 			= $this->formatear($registro->e, "Cadena");
		$this->_email 				= $this->formatear($registro->f, "Cadena");
		$this->_comentario 			= $this->formatear($registro->g, "Cadena");


		$this->_sql = sprintf("INSERT INTO pedido VALUES(NULL ,NOW(),NULL,%s,%s,%s,%s,'Pedido');",
	...
	public function actualizar($registro){
		...
		$this->_id_cliente_pagina 	= $this->formatear($registro->a, "Entero");
		$this->_nombre 				= $this->formatear($registro->d, "Cadena");
		$this->_telefono 			= $this->formatear($registro->e, "Cadena");
		$this->_email 				= $this->formatear($registro->f, "Cadena");
		$this->_comentario 			= $this->formatear($registro->g, "Cadena");
		$this->_origen 				= $this->formatear($registro->h, "Cadena");
		
		
		$this->_sql 			= sprintf("UPDATE pedido SET  
											...
											WHERE pedido.id_cliente_pagina 	=%s LIMIT 1;",
	...
	public function borrar($registro) 
	{
		...
		$this->_sql 		= sprintf("DELETE FROM pedido WHERE pedido.id_cliente_pagina=%s LIMIT 1;",$this->_id_cliente_pagina);
		return $this->sentenciaSQL($this->_sql, 5);

 ...
 *  @final
 */
final Class Equipo extends Base implements peticiones
{
	/*
	...
	public function __construct()
	{
		...
		if(parent::validaSesion()){
			...
			$this->_aliasColumnas =		'equipo.id_equipo 		AS a,
			 							 equipo.created_at 		AS b,
			 							 equipo.updated_at 		AS c,
			 							 equipo.nombre 			AS d,
			 							 equipo.activo 			AS e
			 							';
		}else {
	...
	public function listar($tipo)
	{
		...
		$this->_sql = sprintf("SELECT %s FROM equipo WHERE equipo.id_equipo>=1 ORDER BY equipo.id_equipo DESC;",$this->_aliasColumnas);
	...
	public function buscar($criterio,$columna,$tipo)
	{
		...
		if ($columna == 0){
			...
			$this->_id_equipo 			= $this->formatear($criterio, "CadenaBusqueda");
			$this->_nombre 				= $this->formatear($criterio, "CadenaBusqueda");

			$this->_sql = sprintf("SELECT %s FROM modelo WHERE (modelo.id_modelo LIKE %s OR modelo.nombre LIKE %s) ORDER BY modelo.numero DESC;",$this->_aliasColumnas, $this->_id_equipo, $this->_nombre);
	...
	public function insertar($registro){
		...
		$this->_activo 			= $this->formatear($registro->e, "Cadena");

		$this->_sql = sprintf("INSERT INTO equipo VALUES(		NULL ,
															NOW(),
															NULL ,
															%s,
															%s);",
	...
	public function actualizar($registro){
		...
		$this->_id_equipo 				= $this->formatear($registro->a, "Entero");
		$this->_nombre 					= $this->formatear($registro->d, "Cadena");
		$this->_activo 					= $this->formatear($registro->e, "Cadena");
		
		$this->_sql 			= sprintf 	("	UPDATE equipo SET  
	...
	public function borrar($registro) 
	{
		...
		$this->_sql 		= sprintf("DELETE FROM equipo WHERE equipo.id_equipo=%s LIMIT 1;",$this->_id_equipo);
		return $this->sentenciaSQL($this->_sql, 5);

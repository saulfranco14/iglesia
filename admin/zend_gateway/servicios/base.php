	...
	*/
		...
		if($this->_elDato != '') {
			...
			switch ($this->_elTipo) {
				...
				case "HoraSinSegundos":
					...
					break;
				case "Encriptalo":
					$this->_elDato = "'" . sha1(trim($this->_elDato)) . "'";
					break;
				default:
	...
	protected function hora($laHora, $sinSegundos='')
	{
		...
		if($sinSegundos == 'S') {
			...
			if( (count($_laHora) == 3) and
					($_laHora[0]<24 and $_laHora[0]>=0) and ($_laHora[1]<60 and $_laHora[1]>=0) and 
					($_laHora[2]<60 and $_laHora[2]>=0) ) {
				return $laHora;
			} else {
				return '00:00:00';
			}
	...
	{
		if(isset($_SESSION["id_usuario"]) && isset($_SESSION["tokenUsuario"])) {
			if( $_SESSION["tokenUsuario"] == md5(sha1(session_id() . $_SERVER['REMOTE_ADDR'] . $_SESSION["id_usuario"])) ) {
				return true;
			} else {
				return false;
			}

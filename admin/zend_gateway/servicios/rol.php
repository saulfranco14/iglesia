 ...
 *  @final
 */
final Class Rol extends Base implements peticiones
{
	/*
	...
	public function __construct()
	{
		...
		if(parent::validaSesion()){
			...
			$this->_aliasColumnas =		'rol.id_rol 			AS a,
			 							 rol.created_at 		AS b,
			 							 rol.updated_at 		AS c,
			 							 rol.nombre_rol 		AS d,
			 							 rol.descripcion 		AS e
			 							';
		}else {
	...
	public function listar($tipo)
	{
		...
		$this->_sql = sprintf("SELECT %s FROM rol WHERE rol.id_rol>=1 ORDER BY rol.id_rol DESC;",$this->_aliasColumnas);
	...
	public function buscar($criterio,$columna,$tipo)
	{
		...
		if ($columna == 0){
			...
			$this->_id_rol 			= $this->formatear($criterio, "CadenaBusqueda");
			$this->_nombre_rol 				= $this->formatear($criterio, "CadenaBusqueda");

			$this->_sql = sprintf("SELECT %s FROM modelo WHERE (modelo.id_modelo LIKE %s OR modelo.nombre LIKE %s) ORDER BY modelo.numero DESC;",$this->_aliasColumnas, $this->_id_rol, $this->_nombre_rol);
	...
	public function insertar($registro){
		...
		$this->_nombre_rol	 		= $this->formatear($registro->d, "Cadena");
		$this->_descripcion 		= $this->formatear($registro->e, "Cadena");

		$this->_sql = sprintf("INSERT INTO rol VALUES(	NULL ,
															NOW(),
															NULL ,
															%s,
															%s);",
	...
	public function actualizar($registro){
		...
		$this->_id_rol 				= $this->formatear($registro->a, "Entero");
		$this->_nombre_rol 			= $this->formatear($registro->d, "Cadena");
		$this->_descripcion 		= $this->formatear($registro->e, "Cadena");
		
		$this->_sql 			= sprintf 	("	UPDATE rol SET  
	...
	public function borrar($registro) 
	{
		...
		$this->_sql 		= sprintf("DELETE FROM rol WHERE rol.id_rol=%s LIMIT 1;",$this->_id_rol);
		return $this->sentenciaSQL($this->_sql, 5);
	 ...
	 public function reportePDF($tipo, $criterio='', $columna='')
        ...
        * autopadding=true, $maximaAltura=0, $alineadoVertical='T', $ajustarCelda=false)
         */
         ...
         $this->_reportePDF->Output('../pdfs/'. $_nombre_rol, 'F');
         return $_nombre_rol;
         }}
 ...
 */
  ...
  */
   ...
   */
   ...
   const GATEWAY_PEDIDO = "admin/zend_gateway/index.php";
	...
	*/
	...
	$(document).ready(
      ...
      */
        ...
        $('#tbodyPedido').bind('click', function(event){
...

      {
        ...
        $.ajax({
          ...
          {
            exitolistarPedido(jsonRespuesta, estatusRespuesta, jqXHR);
          },
        ...
        * @returns {void}
        */
        function exitolistarPedido(jsonRespuesta, estatusRespuesta, jqXHR)
        {
          ...
          indice = 0;
          jsonPedido = jsonRespuesta.result;
          $('#tbodyPedido > tr').remove();
          if(jsonPedido.length > 0){
            htmlNuevo = '';
            for(indice=0; indice<jsonPedido.length; indice++){
              objetoPedido = jsonPedido[indice];
              htmlNuevo += '<tr>';
                htmlNuevo += '<td>';
                ...
                htmlNuevo += '</td>';
                htmlNuevo += '<td>' + objetoPedido.a + '</td>';
                htmlNuevo += '<td>' + objetoPedido.b + '</td>';
                htmlNuevo += '<td>' + objetoPedido.c + '</td>';
                htmlNuevo += '<td>' + objetoPedido.d + '</td>';
                htmlNuevo += '<td>' + objetoPedido.e + '</td>';
                htmlNuevo += '<td>' + objetoPedido.f + '</td>';
                htmlNuevo += '<td>' + objetoPedido.g + '</td>';
                htmlNuevo += '<td>' + objetoPedido.h + '</td>';
                htmlNuevo += '</tr>';
            }
            $('#tbodyPedido').append(sanitizarHTML(htmlNuevo));
          }else{
            mostrarVentanaModal('No hay pedido disponible');
          }
          switch (accion){
          case 'buscar':
      ...
      /**
      * Funcion para mostrar el cliente escogido de la lista.
      ...
      */
      ...
      function editarPedido(indiceEscogido)
      {
        objetoPedido = jsonPedido[indiceEscogido];
          $('#tabsMenu a[href="#divTabFormularioPedido"]').tab('show');
          $('#id_cliente_pagina').val(objetoPedido.a);
          $('#created_at').val(objetoPedido.b);
          $('#updated_at').val(objetoPedido.c);
          $('#nombre').val(objetoPedido.d);
          $('#telefono').val(objetoPedido.e);
          $('#email').val(objetoPedido.f);
          $('#comentario').val(objetoPedido.g);
          $('#origen').val(objetoPedido.h);
          $('#h1TituloFormulario').html(sanitizarHTML('Editar un pedido'));
      }
      /**
      ...
      */
      ...
      function agregarPedido()
      {
        ...
        $('#comentario').val('');
        $('#origen').val('');
        $('#h1TituloFormulario').html(sanitizarHTML('Crear un pedido'));
      ...
      */
      ...
      function guardarPedido()
      {
       ...
       peticionJSON = JSON.stringify({
        ...
        $.ajax({
          ...
          success : function(jsonRespuesta, estatusRespuesta, jqXHR)
          {
            exitoGuardadoPedido(jsonRespuesta, estatusRespuesta, jqXHR);
          },
          error : function(jqXHR, estatusError, textoError)
          {
            mostrarErrorJSON(jqXHR, estatusError, textoError);
      ...
      /**
        ...
        */
        ...
        function exitoGuardadoPedido(jsonRespuesta, estatusRespuesta, jqXHR)
        {
            ...
            switch(accion){
              case 'insertar':
                if(jsonRespuesta.result > 0){
                  //si se inserto
                  mostrarVentanaModal('Gracias por registrarte ');
                }else{
                  //no inserto
                  mostrarVentanaModal('Disculpe, en estos momentos tenemos problemas con el servicio. Intente mas tarde.');
                }
              break;
              case 'actualizar':
                if(jsonRespuesta.result == 1) {
                  //si se actualizo
                  mostrarVentanaModal('Pedido  ' + objetoPedido.a + '   actualizado');
                }else{
                  //no se actualizo
                  mostrarVentanaModal('No se pudo actualizar el pedido');
                }
              break;
              default:
                //no se que paso
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabBorrarPedido"]').tab('show');
          objetoPedido = jsonPedido[indiceEscogido];
          htmlNuevo = 'ID: <strong>' + objetoPedido.a + '</strong><br/>Nombre: <strong>' + objetoPedido.d + '</strong>';
          $('#pDatosPedido').html(sanitizarHTML(htmlNuevo));
         }

         function borrarPedido()
         {
          objetoPedido = { a:objetoPedido.a };
          peticionJSON = JSON.stringify({
            'Id' : generarID(),
            'method' : 'borrar',
            'clase' : CLASE_PEDIDO,
          ...
          $.ajax({
            ...
            success : function(jsonRespuesta, estatusRespuesta, jqXHR)
            {
              exitoBorradoPedido(jsonRespuesta, estatusRespuesta, jqXHR);
            },
            error : function(jqXHR, estatusError, textoError)
            {
              mostrarErrorJSON(jqXHR, estatusError, textoError);
      ...
      /**
        ...
        * @returns {void}
      */
        function exitoBorradoPedido(jsonRespuesta, estatusRespuesta, jqXHR)
        {
          //checamoa si existio un error
            ...
            if(jsonRespuesta.result == 1){
              //si se borro:
              mostrarVentanaModal('Pedido   ' + objetoPedido.c + '   Borrado<br />(recuerde: Este borrado no se puede deshacer).');
            }else{
              //no se borro:
              mostrarVentanaModal('Pedido '+ objetoPedido.c + 'No pudo ser borrado.');
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabListaPedido"]').tab('show');
          listarPedido();
         }
         /**
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabBuscarPedido"]').tab('show');
          $('#inputCriterio').val('');

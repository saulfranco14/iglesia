 ...
 */
  ...
  */
   ...
   */
   ...
   var CLASE_ENTREGA = 'entregasalida';
          ...
          if(jsonEntrega.length > 0){
            ...
            for(indice=0; indice<jsonEntrega.length; indice++){
              ...
              htmlNuevo += '<tr>';
                htmlNuevo += '<td>';
                 // htmlNuevo += '<button id="button_editar_'+ indice +'" type="button" aria-label="Editar" class="btn btn-sm btn-warning"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button>';
                  ...
                  htmlNuevo += '<button id="button_borrar_'+ indice + '" type="button" aria-label="Borrar" class="btn btn-sm btn-success"><span class="glyphicon glyphicon-check" aria-hidden="true"></span></button>';
                 // htmlNuevo += '&nbsp; <br class="visible-xs" />';
              ...
              // htmlNuevo += '<td>' + objetoEntrega.c + '</td>';
                ...
                htmlNuevo += '<td class="text-success"><strong>' + objetoEntrega.trabajo + ' pzas </strong></td>';
          ...
          if(jsonEntregaColores.length > 0){
            ...
            for(indice=0; indice<jsonEntregaColores.length; indice++){
              ...
              //  htmlNuevo += '<td><strong>' + objetoEntregaColores.m + ' </strong> </td>';
                ...
                htmlNuevo += '<td class="hidden-xs">' + objetoEntregaColores.p + '</td>';
                htmlNuevo += '</tr>';
            }
      ...
      function exitolistarCombosEntrega(jsonRespuesta, estatusRespuesta, jqXHR){
        ...
        if (jsonRespuesta.error) {
          ...
          if(jsonEntrega.length > 0 ){
            ...
            for(indice=0; indice<jsonEntrega.length; indice++){
                objetoEntrega = jsonEntrega[indice];
                    $('#entrega_id_entrega').append('<option value="'+ objetoEntrega.a + '">'+ objetoEntrega.d + '</option>');            
            }
      
          }else{

             $('#entrega_id_entrega').append('<option value="x">No entregas </option>');
        ...
        */
         ...
         */
         function confirmarBorrado(indiceEscogido)
         {
          ...
          objetoEntrega = jsonEntrega[indiceEscogido];
          htmlNuevo = 'ID: <strong>' + objetoEntrega.a + '</strong><br/>Feha de entrega: <strong>' + objetoEntrega.f + '</strong><br/>Cantidad: <strong>' + objetoEntrega.d + '</strong>';
      ...
      */
        function exitoBorradoEntrega(jsonRespuesta, estatusRespuesta, jqXHR)
        {
          //checamoa si existio un error
            ...
            if(jsonRespuesta.result == 1){
              //si se borro:
              mostrarVentanaModal('Se dio salida exitosamente<br />(recuerde: Esta salida no se puede deshacer).');
            }else{
              //no se borro:
              mostrarVentanaModal('Entrega '+ objetoEntrega.d + 'no pudo darse por salida, intente más tarde.');

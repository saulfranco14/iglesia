 ...
 */
  ...
  */
   ...
   */
   ...
   const GATEWAY_CLIENTE = "../zend_gateway/index.php";
	...
	*/
	...
	$(document).ready(
      ...
      */
        ...
        $('#tbodyCliente').bind('click', function(event){
...

      {
        ...
        $.ajax({
          ...
          {
            exitolistarCliente(jsonRespuesta, estatusRespuesta, jqXHR);
          },
        ...
        * @returns {void}
        */
        function exitolistarCliente(jsonRespuesta, estatusRespuesta, jqXHR)
        {
          ...
          if(jsonCliente.length > 0){
            ...
            for(indice=0; indice<jsonCliente.length; indice++){
              ...
              htmlNuevo += '<tr>';
                ...
                htmlNuevo += '</td>';
                htmlNuevo += '<td>' + objetoCliente.a + '</td>';
                htmlNuevo += '<td>' + objetoCliente.b + '</td>';
                htmlNuevo += '<td>' + objetoCliente.c + '</td>';
                htmlNuevo += '<td>' + objetoCliente.d + '</td>';
                htmlNuevo += '<td>' + objetoCliente.e + '</td>';
                htmlNuevo += '<td>' + objetoCliente.f + '</td>';
                htmlNuevo += '</tr>';
            }
            $('#tbodyCliente').append(sanitizarHTML(htmlNuevo));
          }else{
            mostrarVentanaModal('No hay cliente disponible');
          }
          switch (accion){
          case 'buscar':
      ...
      */
      ...
      function editarCliente(indiceEscogido)
      {
        objetoCliente = jsonCliente[indiceEscogido];
          $('#tabsMenu a[href="#divTabFormularioCliente"]').tab('show');
          $('#id_cliente').val(objetoCliente.a);
          $('#created_at').val(objetoCliente.b);
          $('#updated_at').val(objetoCliente.c);
          $('#nombre').val(objetoCliente.d);
          $('#orden').val(objetoCliente.e);
          $('#telefono_celular').val(objetoCliente.f);
          $('#h1TituloFormulario').html(sanitizarHTML('Editar un cliente'));
      }
      /**
      ...
      */
      ...
      function agregarCliente()
      {
        ...
        $('#h1TituloFormulario').html(sanitizarHTML('Crear un cliente'));
      ...
      */
      ...
      function guardarCliente()
      {
        objetoCliente = {
          ...
          d: $('#nombre').val(),
          e: $('#orden').val(),
          f: $('#telefono_celular').val()
        };
        if (objetoCliente.a == 0) {
          //insertar Pais
          accion = 'insertar';
        }else{
          //actualizar pais
       ...
       peticionJSON = JSON.stringify({
        ...
        $.ajax({
          ...
          success : function(jsonRespuesta, estatusRespuesta, jqXHR)
          {
            exitoGuardadoCliente(jsonRespuesta, estatusRespuesta, jqXHR);
          },
          error : function(jqXHR, estatusError, textoError)
          {
            mostrarErrorJSON(jqXHR, estatusError, textoError);
      ...
      /**
        ...
        */
        ...
        function exitoGuardadoCliente(jsonRespuesta, estatusRespuesta, jqXHR)
        {
            ...
            switch(accion){
              case 'insertar':
                if(jsonRespuesta.result > 0){
                  //si se inserto
                  mostrarVentanaModal('Cliente insertado con el ID '+ jsonRespuesta.result);
                }else{
                  //no inserto
                  mostrarVentanaModal('No se pudo insertar el cliente');
                }
              break;
              case 'actualizar':
                if(jsonRespuesta.result == 1) {
                  //si se actualizo
                  mostrarVentanaModal('Cliente  ' + objetoCliente.a + '   actualizada');
                }else{
                  //no se actualizo
                  mostrarVentanaModal('No se pudo actualizar al cliente');
                }
              break;
              default:
                //no se que paso
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabBorrarCliente"]').tab('show');
          objetoCliente = jsonCliente[indiceEscogido];
          htmlNuevo = 'ID: <strong>' + objetoCliente.a + '</strong><br/>Nombre: <strong>' + objetoCliente.d + '</strong>';
          $('#pDatosCliente').html(sanitizarHTML(htmlNuevo));
         }

         function borrarCliente()
         {
          objetoCliente = { a:objetoCliente.a };
          peticionJSON = JSON.stringify({
            'Id' : generarID(),
            'method' : 'borrar',
            'clase' : CLASE_CLIENTE,
          ...
          $.ajax({
            ...
            success : function(jsonRespuesta, estatusRespuesta, jqXHR)
            {
              exitoBorradoCliente(jsonRespuesta, estatusRespuesta, jqXHR);
            },
            error : function(jqXHR, estatusError, textoError)
            {
              mostrarErrorJSON(jqXHR, estatusError, textoError);
      ...
      /**
        ...
        * @returns {void}
      */
        function exitoBorradoCliente(jsonRespuesta, estatusRespuesta, jqXHR)
        {
          //checamoa si existio un error
            ...
            if(jsonRespuesta.result == 1){
              //si se borro:
              mostrarVentanaModal('Cliente   ' + objetoCliente.c + '   Borrado<br />(recuerde: Este borrado no se puede deshacer).');
            }else{
              //no se borro:
              mostrarVentanaModal('Cliente '+ objetoCliente.c + 'No pudo ser borrado.');
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabListaCliente"]').tab('show');
          listarCliente();
         }
         /**
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabBuscarCliente"]').tab('show');
          $('#inputCriterio').val('');
